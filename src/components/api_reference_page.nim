## isonim-docs Layer 4 — the API reference page frame (M7 deliverable 2).
##
## Wraps `api_reference.renderApiReference`/`renderApiReferenceHtml` in the
## exact same header/nav/main/footer region shape and stable `regionId`
## anchors as `components/markdown_page.renderMarkdownPage` -- reusing
## `shell.nim`'s own class constants and `shell_vm.regionId` -- so a
## `pkApiReference` page carries the identical site chrome (skip link,
## header with search + theme toggle, site nav, footer, search overlay)
## as every other page kind, with the strict three-column API layout as
## its `<main>` body. The tree (`renderApiReferencePage`) and SSR string
## (`renderApiReferencePageHtml`) paths are kept byte-for-byte in
## lock-step, exactly like `markdown_page.nim`.
...
proc renderApiReferencePage*[R, E](r: R; pageTitle: string; vm: ApiReferenceViewModel;
  ...
  r.appendChild(mainEl, renderApiReference[R, E](r, vm))
...
proc renderApiReferencePageHtml*(pageTitle: string; vm: ApiReferenceViewModel;
                                  navigation: NavigationViewModel = NavigationViewModel();
                                  search: SearchViewModel = SearchViewModel();
